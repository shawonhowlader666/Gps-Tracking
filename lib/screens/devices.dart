...
class _DevicePageState extends State<DevicePage> {
  ...
  String _getSensorValue(dynamic deviceId, String name, String? currentValue) {
    ...
    if (deviceId != null) {
      final int? devIdInt = int.tryParse(deviceId.toString());
      if (devIdInt != null) {
    ...
    final cacheKey =
        'sensor_${deviceId ?? 'unknown'}_${displayName.toLowerCase().replaceAll(' ', '_')}';
                    ...
                    Row(
                      children: [
                        Icon(Icons.access_time,
                            size: 14, color: _getRemainingColor(device)),
                        const SizedBox(width: 4),
                        Expanded(
                          child: Text(
